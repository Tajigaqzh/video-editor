...
import { mockVideoFile, mockAudioFile, mockFolderWithFiles, mockMediaItems } from "./mockData";
  ...
  describe("createNestedFolderStructure", () => {
    it("should create a nested folder structure", () => {
      ...
      if (parentFolder.type === "folder") {
        const parentFolderTyped =
          parentFolder as import("@/utils/media/mediaOperations").MediaFolder;
        expect(parentFolderTyped.children).toHaveLength(1);

        const childFolder = parentFolderTyped.children[0];
        if (childFolder.type === "folder") {
          const childFolderTyped =
            childFolder as import("@/utils/media/mediaOperations").MediaFolder;
